...
import { GlassCard, ScoreRing, MiniBar } from '@/components/shared';
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16, marginBottom: 24 }}>
        ...
        <GlassCard>
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 16 }}>
            <span style={{ fontSize: 48, fontWeight: 800, color: C.warning, lineHeight: 1 }}>7</span>
            <span style={{ fontSize: 16, color: C.textSecondary }}>days</span>
          </div>
          <div style={{ display: 'flex', gap: 6 }}>
            {['M', 'T', 'W', 'T', 'F', 'S', 'S'].map((d, i) => (
              <div key={i} style={{
                flex: 1, height: 28, borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center',
                background: i < 7 ? `${C.warning}25` : C.bgCard,
                border: `1px solid ${i < 7 ? C.warning + '40' : 'transparent'}`,
                fontSize: 10, color: i < 7 ? C.warning : C.textMuted, fontWeight: 600,
              }}>{d}</div>
            ))}
          </div>
        </GlassCard>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {['1W', '1M', '3M'].map((t, i) => (
                <button key={t} style={{
                  ...
                  color: i === 0 ? C.primary : C.textMuted, fontFamily: 'Inter, sans-serif',
                }}>{t}</button>
              ))}
            </div>
          </div>
              ...
              <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                <div style={{
                  ...
                  background: i === 6
                    ...
                    : `linear-gradient(to top, ${C.primary}40, ${C.primary}60)`,
            ...
            {[
              ...
              <div key={label}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                  ...
                  <span style={{ fontSize: 12, color: good ? C.success : C.warning, fontFamily: 'JetBrains Mono' }}>{pct}%</span>
                </div>
                <div style={{ height: 4, borderRadius: 2, background: 'rgba(255,255,255,0.06)' }}>
                  <div style={{
                    height: '100%', borderRadius: 2, width: `${pct}%`,
                    background: good ? `linear-gradient(90deg, ${C.success}80, ${C.success})` : `linear-gradient(90deg, ${C.warning}80, ${C.warning})`,
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {recentSessions.map((s, i) => (
              <div key={i} style={{
                ...
                background: C.bgCard, borderRadius: 10, border: `1px solid ${C.borderMid}`,
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 2, color: C.textPrimary }}>{s.role}</div>
                  <div style={{ fontSize: 11, color: C.textMuted }}>{s.date} · {s.type}</div>
                </div>
                <div style={{
                  fontSize: 20, fontWeight: 700,
                  color: s.score >= 85 ? C.success : s.score >= 70 ? C.primary : C.warning,
                }}>{s.score}</div>
              </div>
            ))}
          </div>

...
const LobbyPage = () => {
  ...
  const statusColor = { available: C.success, busy: C.warning, 'in-interview': C.error };
  ...
  const sendRequest = (user) => {
    ...
    setTimeout(() => { setActiveReq(null); router.push('/p2p'); }, 2200);
  ...
  return (
    <div style={{ flex: 1, height: '100vh', overflow: 'auto', padding: '28px 32px', position: 'relative' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 28 }}>
        <div>
          <h2 style={{ fontSize: 26, fontWeight: 800, marginBottom: 4, display: 'flex', alignItems: 'center', gap: 10 }}>
            Peer Arena
            <div style={{ width: 8, height: 8, borderRadius: '50%', background: C.success, boxShadow: `0 0 10px ${C.success}`, marginTop: 2 }} />
          </h2>
          <p style={{ color: C.textSecondary, fontSize: 14 }}>
            <span style={{ color: C.success, fontWeight: 700 }}>6 online</span> · Connect with real users for live mock interviews
          </p>
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          <NeonButton variant="outline" size="sm" onClick={() => router.push('/dashboard')}>← Dashboard</NeonButton>
          <NeonButton size="sm" onClick={() => router.push('/p2p')}>Quick Match ⚡</NeonButton>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 24 }}>
        {[
          { label: 'Online Now', value: '6', color: C.success, icon: '◉' },
          { label: 'Active Rooms', value: '3', color: C.primary, icon: '◈' },
          { label: 'Available HR', value: '3', color: C.secondary, icon: '🎙' },
          { label: 'Avg Wait', value: '< 2m', color: C.warning, icon: '⏱' },
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 20, alignItems: 'center' }}>
        <div style={{
          ...
          background: C.bgCard, border: `1px solid ${C.borderMid}`, borderRadius: 10, padding: '10px 14px',
        }}>
          <span style={{ color: C.textMuted }}>🔍</span>
          <input
            value={search} onChange={e => setSearch(e.target.value)}
            placeholder="Search by name or focus area…"
            style={{ flex: 1, background: 'none', border: 'none', outline: 'none', color: C.textPrimary, fontFamily: 'Space Grotesk', fontSize: 13 }}
          />
        </div>
        <div style={{ display: 'flex', gap: 4, background: C.bgCard, borderRadius: 10, padding: 4, border: `1px solid ${C.borderMid}` }}>
          {[['all','All'],['available','Available'],['hr','HR / Interviewer'],['candidate','Candidate']].map(([v, l]) => (
            <button key={v} onClick={() => setFilter(v)} style={{
              padding: '7px 14px', borderRadius: 8, border: 'none', cursor: 'pointer',
              background: filter === v ? `linear-gradient(135deg, rgba(167, 139, 250, 0.2), rgba(139, 92, 246, 0.2))` : 'transparent',
              color: filter === v ? C.primary : C.textMuted,
        ...
        {filtered.map(u => (
          <GlassCard key={u.id} hover style={{ padding: '20px 18px', textAlign: 'center', position: 'relative' }}>
            ...
            <div style={{
              ...
              border: `2px solid ${u.status === 'available' ? C.borderMid : C.borderMid}`,
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              margin: '0 auto 10px',
              boxShadow: u.status === 'available' ? `0 0 16px rgba(167, 139, 250, 0.2)` : 'none',
            }}>{u.avatar}</div>
            ...
            <div style={{ display: 'flex', gap: 5, justifyContent: 'center', flexWrap: 'wrap', marginBottom: 12 }}>
              <Tag color={u.role === 'interviewer' ? C.secondary : C.primary}>{u.role === 'interviewer' ? 'HR / Interviewer' : 'Candidate'}</Tag>
              <Tag color={C.textMuted}>{u.focus}</Tag>
            </div>
            <div style={{ display: 'flex', justifyContent: 'center', gap: 16, marginBottom: 14 }}>
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: 18, fontWeight: 800, color: C.primary }}>{u.score}</div>

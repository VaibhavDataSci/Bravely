  ...
  return (
    <div style={{ flex: 1, padding: '40px 48px', overflowY: 'auto' }}>
      ...
      <div style={{ marginBottom: 40, textAlign: 'center' }}>
        <h1 style={{ fontSize: 32, fontWeight: 800, color: C.text, marginBottom: 12, letterSpacing: '-0.02em' }}>
          What role do you want to practice today?
        </h1>
        <p style={{ color: C.textDim, fontSize: 16, maxWidth: 640, margin: '0 auto', lineHeight: 1.5 }}>
      ...
      <div style={{ 
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          <div style={{ display: 'flex', gap: 8, background: 'rgba(255,255,255,0.02)', padding: 6, borderRadius: 12, border: `1px solid ${C.border}` }}>
            {DIFFICULTIES.map(d => (
              <button
                key={d}
                ...
                style={{
                  ...
                  color: difficulty === d ? C.text : C.textDim,
                  border: 'none',
                  cursor: 'pointer',
            ...
            {(selectedRole ? CONTEXTS_BY_ROLE[selectedRole.id] : CONTEXTS_BY_ROLE.default).map(c => (
              <button
                ...
                style={{
                  ...
                  color: interviewContext === c ? C.text : C.textDim,
                  border: 'none',
                  cursor: 'pointer',
                  transition: 'all 0.2s ease',
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: 16,
        border: `1px solid ${isSelected ? C.cyan : (hover ? 'rgba(6,182,212,0.5)' : C.border)}`,
        transform: isSelected ? 'scale(1.05)' : (hover ? 'translateY(-4px)' : 'scale(1)'),
        boxShadow: isSelected ? `0 0 32px ${C.cyan}40, inset 0 0 16px ${C.cyan}10` : (hover ? `0 12px 32px rgba(6,182,212,0.15)` : 'none'),
        transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
        textAlign: 'center',
        cursor: 'pointer'
      }}
    >
      ...
      <h3 style={{ fontSize: 18, fontWeight: 700, color: isActive ? C.text : C.textDim, marginBottom: 12, transition: 'color 0.3s ease' }}>
        {role.name}
      </h3>

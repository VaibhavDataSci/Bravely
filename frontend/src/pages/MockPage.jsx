...
const MockPage = () => {
  ...
  const [liveFeedback, setLiveFeedback] = React.useState([
    { text: 'Strong opening statement', color: C.green, t: '00:12' },
    { text: 'Avoid filler words', color: C.amber, t: '00:38' },
    { text: 'Good technical depth', color: C.green, t: '01:15' },
  ]);
  ...
  return (
    <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 360px', height: '100vh', overflow: 'hidden', background: '#030712' }}>
      ...
      <VideoCall roomId={roomId || 'mock-default'} userId={user?.email || `user-${Math.floor(Math.random()*1000)}`} mode="mock" onEndCall={() => navigate('/report')} />
      ...
      <div style={{
        ...
        <div style={{ padding: 16, borderBottom: `1px solid ${C.border}` }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            {[
              { l: 'Confidence', v: 78, c: C.cyan },
              { l: 'Relevance', v: 85, c: C.green },
              { l: 'Fluency', v: 62, c: C.amber },
              { l: 'Depth', v: 90, c: C.violetBright },
            ].map(({ l, v, c }) => (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {liveFeedback.map((f, i) => (
              <div key={i} style={{
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 12, color: C.text, lineHeight: 1.4 }}>{f.text}</div>
        ...
        <div style={{ padding: 16, borderTop: `1px solid ${C.border}` }}>
          ...
          <div style={{ background: C.glass, borderRadius: 8, padding: '12px 14px', fontSize: 12, color: C.textDim, lineHeight: 1.5, border: `1px solid ${C.border}` }}>
